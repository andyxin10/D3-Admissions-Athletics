...
// rename institutionname to school (matches variable name for d3 schools dataset)
...
// admits and enrolled both correlated with applicants and yield
drop admits
drop enrolled

// I am using total SAT score, so individual breakdowns are redundant
drop sat_math
drop sat_verbal
...
* Merge with D3 schools. Intend to filter ipeds dataset to only contain data for Division III schools *
...
// Rename some schools to match school names in the sport-specific datasets
...
* Merge D3 ipeds with sport qualification data *
...
// Create lagged athletics success variables. Lagged by 2 years (see ReadMe)
...
* Merge D3 + ipeds + NCAA with sport-specific expenses

// Will focus on expenses, exclude revenues


import delimited "/Users/andyxin/Desktop/ECON135/FinalProject/expenses.csv", clear

// School names slightly differ. However, this dataset has unitid, which is unique to each school
drop school
...
* Prior to regression analysis, wish to test whether pre-trends assumption is satisfied (yield rate is not decreasing / acceptances is not increasing prior to event of interest)
...
// Summary Statistics (Academics)

estpost summarize yield_rate acceptance_rate applicants tuition_adj retention student_faculty_ratio pct_finaid sat75
esttab using "/Users/andyxin/Desktop/ECON135/FinalProject/admission_summary.tex", replace ///
cells("mean(fmt(2)) sd(fmt(2)) min(fmt(2)) max(fmt(2))") ///
title("Summary Statistics: Academics and Admissions")

// Summary Statistics (Athletics Performance)

preserve

// Restrict years. Years after 2023 are not counted in overall study due to 2 year lag

keep if year >= 2013 & year <= 2022

// School-Level
bysort unitid year: egen conf_any = max(any_confchamp)
bysort unitid year: egen ncaa_any = max(any_ncaa)
...
// Regression tables for the C+H method were not compiling so I asked AI to write up the table (in LaTeX) given the regression results. This goes for all did_multiplegt_stat regressions as well.



* Regressions by Sport. Determine whether success effect differs between sports
...
* Regressions by Gender, determine whether success effect differs between men's and women's sports
...
* Robustness check: Incorporate expenses
